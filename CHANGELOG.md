...
## \[[0.700.0](https://github.com/holochain/scaffolding/compare/v0.600.0...v0.700.0)\] - 2026-07-31

### Features

- Update to Holochain 0.7.0 by @ThetaSinner in [#588](https://github.com/holochain/scaffolding/pull/588)
- \[**BREAKING**\] Update scaffolded validate() codegen for Holochain 0.7.0-rc.5 by @ThetaSinner in [#587](https://github.com/holochain/scaffolding/pull/587)
  - Bumps holochain to 0.7.0-rc.5 and hdi/hdk to 0.8.0-rc.4/0.7.0-rc.4, and updates the syn/quote codegen to the TypedAction API that holochain#5910 introduced. The two changes are atomic: the old codegen does not compile against hdi 0.8.0-rc.4 (the OpEntry/OpActivity/OpRecord `agent()`, `new_key()` and `original_key()` accessors were removed), and the new codegen does not compile against rc.3 (`try_from_action` did not exist yet).
  - The API additions replace boilerplate this generator was emitting:
  - `TypedAction<CreateData>` and `TypedAction<UpdateData>` now convert   infallibly into `TypedAction<EntryCreationData>`, so the round trip   through `Action` plus `try_into` plus `map_err` collapses to `.into()`. - `TypedAction::<D>::try_from_action` returns `ExternResult`, replacing   the `try_into` plus `map_err(|e| wasm_error!(...))` pairs and the   hand-rolled `ActionData::CreateLink` match in the DeleteLink arm. - `OpActivity::CreateAgent` carries an `agent` field again, so the   `Option`-returning accessor and its unreachable error arm are gone. - `TypedAction` derefs to its data, so `action.data.x` becomes   `action.x` wherever the field is only read or cloned.
  - Two things worth noting for review:
  - The `OpUpdate::Entry` arm now fetches the original record once in the   match scaffold and lets each entry type arm reuse it, instead of every   arm issuing its own `must_get_valid_record` for the same address. - `validate_referenceable` deliberately keeps `action.data.base_address`   rather than using the new deref, because `into_entry_hash` and   `into_action_hash` take self by value and you cannot move out of a   deref. Recorded as a comment so it is not "simplified" later.
  - A DeleteLink whose target is not a CreateLink now propagates an error instead of returning `ValidateCallbackResult::Invalid`, following the guidance holochain#5910 added: sys validation already guarantees that shape, so blaming the data's author for the violation is wrong.
  - Verified with run_test.sh, which passes end to end including the 22 sweettest zome tests, the wasm32 build and packaging, and clippy on the generated app. Also confirmed the bare "no entry or link types yet" integrity zome skeleton still compiles, which run_test.sh never reaches.
...
- *(link-type)* Compare link targets as any linkable hashes by @veeso in [#575](https://github.com/holochain/scaffolding/pull/575)
...
- Add missing perl package to default devShell by @cdunster
...
- *(deps)* Bump the nix group across 1 directory with 3 updates by @dependabot[bot] in [#581](https://github.com/holochain/scaffolding/pull/581)
- Update dependabot.yml with shared content in [#578](https://github.com/holochain/scaffolding/pull/578)
- Update CODEOWNERS with shared content in [#572](https://github.com/holochain/scaffolding/pull/572)

### First-time Contributors

- @dependabot[bot] made their first contribution in [#581](https://github.com/holochain/scaffolding/pull/581)
- @veeso made their first contribution in [#575](https://github.com/holochain/scaffolding/pull/575)
