    ...
    <div style="display: flex; flex-direction: row; flex: 1">
          ...
          <div style="position: absolute; top: 0; left: 0; right: 0; bottom: 0">
            <div style="max-height: 100%; overflow-y: auto">
              <mwc-list activatable>
                <mwc-list-item
                  v-for="(entryDef, entryDefIndex) of integrityZome.entry_defs"
                  :key="entryDefIndex"
                  graphic="icon"
      ...
      >
        <mwc-button
          ...
          :disabled="integrityZome.entry_defs.length < 2"
          icon="delete"
...
import { newCoordinatorZomeDef, newEntryDef, newIntegrityZomeDef, IntegrityZomeDefinition } from '@holochain-scaffolding/definitions';
...
export default defineComponent({
  ...
  props: {
    integrityZome: { type: Object as PropType<IntegrityZomeDefinition>, required: false, default: newIntegrityZomeDef('zome_0') },
  ...
  computed: {
    otherEntryDefsNames() {
      return this.integrityZome.entry_defs
    ...
    selectedEntryDef() {
      ...
      else return this.integrityZome.entry_defs[this.selectedEntryDefIndex];
    },
  },
  mounted() {
    this.updateZomeName();
  ...
  methods: {
    updateZomeName() {
      ...
      setTimeout(() => {
        ...
        field.value = this.integrityZome.name;
      }, 1);
    },
    addEntryDef() {
      const name = `entry_def_${this.entryDefCount++}`;
      this.integrityZome.entry_defs.push(newEntryDef(name));
      this.selectedEntryDefIndex = this.integrityZome.entry_defs.length - 1;
      this.emitChanged();
    },
    deleteEntryDef() {
      this.integrityZome.entry_defs.splice(this.selectedEntryDefIndex, 1);
    ...
    setZomeName(textfield: TextField) {
      if (textfield.validity.valid) {
        this.integrityZome.name = textfield.value;
      }
      this.emitChanged();
    },
    emitChanged() {
      this.$forceUpdate();
      this.$emit('zome-changed', this.integrityZome);
    },
  },
});
</script>
