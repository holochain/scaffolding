...
static SVELTE_PARTIALS: Dir<'static> =
    include_dir!("$CARGO_MANIFEST_DIR/templates/uis/svelte/partials");
static CREATE_ENTRY_COMPONENT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/templates/uis/svelte/create-entry.hbs"
));
static ENTRY_DETAIL_COMPONENT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/templates/uis/svelte/entry-detail.hbs"
));
...
pub fn create_entry_component(data: &AddEntryTypeComponentsData) -> ScaffoldResult<String> {
    ...
    let h = register_all_partials_in_dir(h, &SVELTE_PARTIALS)?;
...
pub fn add_package_json_dependencies(
    ...
    for (_name, field_definition) in entry_def.fields.clone() {
        if let Some(template) = SVELTE_PARTIALS.get_file(format!(
            "{}/package.json.hbs",
