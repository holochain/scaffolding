...
static LIT_PARTIALS: Dir<'static> = include_dir!("$CARGO_MANIFEST_DIR/templates/uis/lit/partials");
static CREATE_ENTRY_COMPONENT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/templates/uis/lit/create-entry.hbs"
));
static ENTRY_DETAIL_COMPONENT: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/templates/uis/lit/entry-detail.hbs"
));
...
pub fn create_entry_component(data: &AddEntryTypeComponentsData) -> ScaffoldResult<String> {
    ...
    let h = register_all_partials_in_dir(h, &LIT_PARTIALS)?;
...
pub fn add_package_json_dependencies(
    ...
    for (_name, field_definition) in entry_def.fields.clone() {
        if let Some(template) = LIT_PARTIALS.get_file(format!(
            "{}/package.json.hbs",
...
pub fn add_entry_components(
    ...
    app_file_tree
        ...
        .ok_or(ScaffoldError::PathNotFound(folder_path.clone()))?
        .insert(
            OsString::from(format!("{}-detail.ts", entry_def.name.to_case(Case::Kebab))),
